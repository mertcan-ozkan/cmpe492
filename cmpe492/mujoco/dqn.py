...
# Online_Net = torch.load("e_4000_decay100_target_net_222")
# Online_Net.eval()
# Target_Net = torch.load("e_4000_decay100_target_net_h")
# Target_Net.eval()
...
for episode in range(4001):  
    ...
    if episode % 500 == 0 :
        with open("e_" +str(episode) +"plot_buffer_4000_222_ppo.npy", 'wb') as f:
            np.save(f, plot_buffer)
        with open("e_" +str(episode) +"replay_buffer_4000_222_ppo.npy", 'wb') as f:
            np.save(f, replay_buffer)
        with open("e_" +str(episode) +"rew_buffer_4000_222_ppo.npy", 'wb') as f:
            np.save(f, rew_buffer)
        with open("e_" +str(episode) +"epsilon_4000_222_ppo.npy", 'wb') as f:
            arr  = np.zeros(1)
            arr[0] = EPSILON
            np.save(f, arr)
...
plt.xlabel("Episodes")
plt.ylabel("Rewards")
plt.savefig('ppo_mujoco_decay100_222.png')
plt.show()
