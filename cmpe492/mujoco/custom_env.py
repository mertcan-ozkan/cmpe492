...
class CustomEnv(gym.Env):
    def __init__(self, render_mode=None) -> None:
        ...
        self._joint_names = [
            ...
            "ur10e/wrist_2_joint",
            "ur10e/wrist_3_joint",
            "ur10e/robotiq_2f85/right_driver_joint"
        ]
        self._reset()
        
        self._joint_qpos_idxs = [self.model.joint(x).qposadr for x in self._joint_names]
        self._ee_site = "ur10e/robotiq_2f85/gripper_site"
    ...
    def _create_scene(self):
        ...
        #            1.5]
        goal_pos = [np.random.uniform(-1.1 , -0.60),
        np.random.uniform(-0.2 ,0.50),
        1.025]

        create_visual(scene, "cylinder", pos=goal_pos, quat=[0, 0, 0, 1],
    ...
    def high_level_state(self):
        ...
        return np.concatenate([ee_pos, goal_pos])
    ...
    def reward(self):
        ...
        distance = -1*np.sqrt(np.sum((ee_pos - goal_pos)**2))
    ...
    def step(self, action_id):
        ...
        target_pos[:2] = np.clip(target_pos[:2] + action,  [-1.1, -0.2 ], [-0.60, 0.50])
        ...
        if self.time == 4000:  
                ...
                with open('plot_buffer_4000_222_ppo.npy', 'wb') as f:
                    np.save(f, self.plot_buffer)
                with open('len_buffer_4000_222_ppo.npy', 'wb') as ff:
                ...
                plt.ylabel("Rewards")
                plt.savefig('mujoco_ppo_4000_222_ppo.png')
        ...
        if  terminal or truncated  :   
            ...
            return np.concatenate([ee_pos, goal_pos]), reward, True, {}
        else: 
            return np.concatenate([ee_pos, goal_pos]), reward, False, {}
    def reset(self):
        self._reset()
        ee_pos = self.data.site(self._ee_site).xpos[:2]
        goal_pos = self.data.site("goal").xpos[:2]
...
def create_tabletop_scene():
    scene = create_empty_scene() # create_base(root, position, height, rgba=[0.5, 0.5, 0.5, 1.0]):
    x = -1.4
    y = 0.15
    add_camera_to_scene(scene, "frontface",  [2.5  +x, 0.  +y , 2.0   ], [-1.5 +x, 0+y, 0])
    add_camera_to_scene(scene, "topdown",    [0.73 +x, 0.  +y , 2.3   ], [0.68 +x, 0+y, 0])
    create_base(scene,                       [0    +x, 0   +y , 0.5    ], 0.5)
    create_object(scene, "box",              [0.7  +x, 0   +y , 1      ], [0, 0, 0, 1],      [0.5, 0.5, 0.02], [0.7, 0.7, 0.7, 1.0], friction=[0.2, 0.005, 0.0001], name="table", static=True)
    create_object(scene, "box",              [0.7  +x, 0   +y , 0.5    ], [0, 0, 0, 1],      [0.05, 0.05, 0.5], [0.9, 0.9, 0.9, 1.0], name="table_leg", static=True)
    create_object(scene, "capsule",          [0.7  +x, 0.5 +y , 1.04 ], [0, 0.7071068, 0, 0.7071068],  [0.02, 0.5], [0.3, 0.3, 1.0, 1.0], name="right_wall", static=True)
    create_object(scene, "capsule",          [0.7  +x, -0.5+y , 1.04], [0, 0.7071068, 0, 0.7071068],   [0.02, 0.5], [0.3, 0.3, 1.0, 1.0], name="left_wall", static=True)
    create_object(scene, "capsule",          [0.2  +x, 0.  +y , 1.04  ], [0.7071068, 0.7071068, 0, 0], [0.02, 0.5], [0.3, 0.3, 1.0, 1.0], name="top_wall", static=True)
    create_object(scene, "capsule",          [1.2  +x, 0.  +y , 1.04  ], [0.7071068, 0.7071068, 0, 0], [0.02, 0.5], [0.3, 0.3, 1.0, 1.0], name="bottom_wall", static=True)
    scene.find("site", "attachment_site").attach(create_ur10e_robotiq85f())
    return scene
...
def create_ur10e_robotiq85f():
    robot = mjcf.from_path("mujoco_menagerie/universal_robots_ur10e/ur10e.xml")
